...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, moeflag=False, num_experts=11):
        ...
        if self.moeflag:
            self.conv1 = nn.ModuleList([conv3x3(inplanes, planes, stride) for _ in range(self.num_experts)])
            self.conv2 = nn.ModuleList([conv3x3(planes, planes) for _ in range(self.num_experts)])
        else:
    ...
    def forward(self, x):
        identity = x
        expert = global_var.get_value('expert')
        if self.moeflag:
            out = self.conv1[expert](x)
        else:
            out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
...
def cifar100_resnet20_eval() -> CifarResNet:
